...
function authnAndExecute(address, username, password, command, ain, parameters, callback) {
	...
	httpGetAsync(url, 
		function (challengeData) {
			...
			var xmlDoc = parser.parseFromString(challengeData, "text/xml");
...
function isInvalidSID (sid) {
	// A SID only containing '0' is invalid
	return new RegExp("^[0]+$").test(sid);
}
...
function MD5 (string) {
   ...
   x = ConvertBytesToWordArray(toUTF16LE(string));
