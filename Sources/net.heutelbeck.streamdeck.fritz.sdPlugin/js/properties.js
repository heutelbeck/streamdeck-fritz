...
function connectElgatoStreamDeckSocket (inPort, inUUID, inRegisterEvent, inInfo, inActionInfo) {
    ...
    console.log("inActionInfo: ", actionInfo);
	...
	loadSettings(settings,actionInfo.context);
...
function loadSettings(settings,context) {
	console.log("loading settings into property inspector for: ",context);
	...
	if(settings.hasOwnProperty('password')) {
		document.getElementById('userpasswordinput').value = settings['password'];
	}
	if(settings.hasOwnProperty('ain'+context)) {
		document.getElementById('aininput').value = settings['ain'+context];
	}
	if(settings.hasOwnProperty('action'+context)) {
		var valueToSet = settings['action'+context];
		var selectObj = document.getElementById('action_select');
		for (var i = 0; i < selectObj.options.length; i++) {
			console.log(((selectObj.options[i].value == valueToSet)?'*':' ')+" selectObj.options["+i+"]="+selectObj.options[i].value);
			selectObj.options[i].selected = (selectObj.options[i].value == valueToSet);		
		}
	}
	if(settings.hasOwnProperty('parameters'+context)) {
		document.getElementById('parametersinput').value = settings['parameters'+context];
	}
}
